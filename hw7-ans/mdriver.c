...
const malloc_impl_t* test[] =
   {&wrapped_impl, &aligned_impl, &fixed_impl, &smart_impl};
...
int main(int argc, char **argv) {
   ...
   */
  while ((c = getopt(argc, argv, "f:t:hvVgalbcWAFS")) != EOF) {
    switch (c) {
  ...
  if (run_base) {
     ...
     for (i = 0; i < num_tracefiles; i++) {
        ...
        if (verbose > 1)
           printf("Checking base malloc for correctness, ");
        base_stats[i].valid = eval_mm_valid(&base_impl, trace, i);
        if (check_heap) {
  ...
  for (i = 0; i < num_tracefiles; i++) {
    if (mm_stats[i].valid) {
      ...
      double baseimpl_throughput = base_stats ? base_stats[i].ops / base_stats[i].secs : 0;
      ...
      if (verbose) {
        printf("%30s%8.0f%8.0f%8.0f%6.0f%%%6.0f%%\n",
               tracefiles[i], baseimpl_throughput/1000, base_throughput/1000,
...
static void usage(void) {
  ...
  fprintf(stderr, "\t-A         use aligned malloc.\n");
  fprintf(stderr, "\t-F         use fixed malloc.\n");
